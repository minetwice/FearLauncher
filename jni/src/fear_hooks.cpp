...
const unsigned char* fear_glGetString(unsigned int name) {
    if (name == GL_VERSION) {
        return (const unsigned char*)"4.6 (Core Profile) Fear Virtual Desktop Engine";
    } else if (name == GL_RENDERER) {
        return (const unsigned char*)"NVIDIA GeForce GTX 1080 (Fear Virtual Desktop Engine)";
    } else if (name == GL_VENDOR) {
        return (const unsigned char*)"Fear Open-Source Technologies";
