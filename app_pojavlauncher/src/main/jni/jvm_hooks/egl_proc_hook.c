...
static OSMesaContext fear_safe_osmesa_create(OSMesaContext share) {
    if (!OSMesaCreateContext_p) {
        printf("eglCreateContext: OSMesaCreateContext_p is NULL\n");
        fflush(stdout);
        return NULL;
    }
    ...
    if (sigsetjmp(g_osmesa_jmp, 1) == 0) {
        printf("eglCreateContext: calling OSMesaCreateContext_p=%p\n", (void*)OSMesaCreateContext_p);
        fflush(stdout);
        ctx = OSMesaCreateContext_p(GL_RGBA, share);
        printf("eglCreateContext: OSMesaCreateContext returned %p\n", (void*)ctx);
        fflush(stdout);
    } else {
        printf("eglCreateContext: OSMesaCreateContext SIGSEGV caught\n");
        fflush(stdout);
        ctx = NULL;
    }
    g_osmesa_guard = 0;
...
EGLContext eglCreateContext(EGLDisplay dpy, EGLConfig config,
    ...
    /* softpipe FIRST — Zink SIGSEGV leaves Mesa corrupted for later tries */
    printf("eglCreateContext: trying softpipe first (stable path)...\n");
    fflush(stdout);
    ...
    if (!octx) {
        printf("eglCreateContext: softpipe failed, trying llvmpipe\n");
        fflush(stdout);
    ...
    if (!octx) {
        printf("eglCreateContext: software failed, trying zink last\n");
        fflush(stdout);
    ...
    if (!octx) {
        printf("eglCreateContext: trying Mesa default driver (no override)\n");
        fflush(stdout);
    ...
    if (!octx) {
        printf("eglCreateContext: all OSMesa create paths failed\n");
        fflush(stdout);
        egl_error = EGL_BAD_ALLOC;
        return EGL_NO_CONTEXT;
    }
    ...
    win->context = octx;
    printf("eglCreateContext: OK win=%p osmesa=%p\n", (void*)win, (void*)octx);
    fflush(stdout);
...
EGLBoolean eglDestroyContext(EGLDisplay dpy, EGLContext ctx) {
    if (is_zink_renderer() && ctx) {
        ...
        if (win->context && OSMesaDestroyContext_p)
...
EGLBoolean eglMakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read, EGLContext ctx) {
    ...
    printf("eglMakeCurrent: osm_make_current win=%p native=%p\n",
           (void*)win, (void*)win->newNativeSurface);
    fflush(stdout);
