...
/* SIGSEGV guard around OSMesaCreateContext (Zink/Vulkan init can crash in strtoul) */
...
EGLContext eglCreateContext(EGLDisplay dpy, EGLConfig config,
    ...
    printf("eglCreateContext: trying OSMesaCreateContext (zink)...\n");
    fflush(stdout);
    OSMesaContext octx = fear_safe_osmesa_create(share_ctx);

    if (!octx) {
        printf("eglCreateContext: Zink failed, falling back to softpipe\n");
        fflush(stdout);
