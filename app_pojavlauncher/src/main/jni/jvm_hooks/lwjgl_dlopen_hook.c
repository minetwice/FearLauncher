...
static unsigned int get_bound_buffer_id(unsigned int target) {
    ...
    static getIntegerv_pfn real_glGetIntegerv = NULL;
...
// ============================================================================
// glMapBufferRange hook — ALWAYS shadow buffer, NEVER call real glMapBufferRange
// ============================================================================
...
static void* glMapBufferRange_hook(unsigned int target, long offset, long length, unsigned int access) {
    ...
    // ALWAYS shadow buffer — never attempt real glMapBufferRange
    ...
    if (!ptr) {
        LOGE("LWJGL linkerhook: glMapBufferRange_hook MALLOC FAILED len=%ld", alloc_len);
        return NULL;
    }
...
void installLwjglDlopenHook(JNIEnv *env) {
    LOGI("Installing LWJGL dlopen() and dlsym() hooks (BUILD v20260907-D)");
