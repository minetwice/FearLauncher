...
int glUnmapBuffer(unsigned int target) {
    ...
    if (found_slot >= 0) {
        ...
        if (entry.shadow_ptr && entry.length > 0) {
            ...
            if (real_glBindBuffer) real_glBindBuffer(target, entry.buffer_id);
...
void installLwjglDlopenHook(JNIEnv *env) {
    ...
    // === Install native bytehook to catch glfwGetProcAddress/eglGetProcAddress bypass ===
    printf("LWJGL linkerhook: Installing native bytehook for egl/glfw GetProcAddress...\n");
    install_global_egl_hook();
    printf("LWJGL linkerhook: Native bytehook installation attempted.\n");
}
