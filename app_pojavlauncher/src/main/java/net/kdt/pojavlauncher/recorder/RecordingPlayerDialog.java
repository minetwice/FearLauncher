 ...
 * play/pause, a real STOP button, a seek bar 
 * and a time readout.
 */
public final class RecordingPlayerDialog {}
    ...
    public static void show(final Context context, final RecordingStore.Entry entry) {
        ...
        final VideoView videoView = dialog.findViewById(R.id.rec_player_video);
         Button playPause = dialog.findViewById(R.id.rec_player_play);        Button stop = dialog.findViewById(R.id.rec_player_stop);
        Button close = dialog.findViewById(R.id.rec_player_close);        final SeekBar seek = dialog.findViewById(R.id.rec_player_seek);        final TextView time = dialog.findViewById(R.id.rec_player_time);
        if (entry.uri != null) videoView.setVideoURI(entry.uri);
        else if (entry.path != null) videoView.setVideoPath(entry.path);
        final Handler handler = new Handler(Looper.getMainLooper());        final Runnable tick = new Runnable() {
            @Override
            public void run() {
                try {
        ...
        videoView.setOnPreparedListener(mp -> {
            try {
                mp.setLooping(false);
                videoView.start();                playPause.setText("PAUSE");            } catch (Exception ignored) {            }
        ...
        seek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
         ...
         playPause.setOnClickListener(v -> {
            try {
                if (videoView.isPlaying()) {
                    videoView.pause();                     playPause.setText("PLAY");
                } else {
                    videoView.start();                     playPause.setText("PAUSE");
                }
            } catch (Exception ignored) {
            }
        });
        stop.setOnClickListener(v -> {
            ...
            playPause.setText("PLAY");            time.setText("STOPPED — close and reopen to play again");
        });

        close.setOnClickListener(v -> dialog.dismiss());
        dialog.setOnDismissListener(d -> {
            handler.removeCallbacks(tick);            try {                videoView.stopPlayback();            } catch (Exception ignore) {
                }
            }
        });
