...
public class JREUtils {
    ...
    public static void setupRendererEnv(Map<String, String> envMap, String renderer) {
        switch(renderer) {
            case "fear_xextream":
                Logger.appendToLog("[FearXextream] Initializing High-FPS GLES Engine Environment...");
    ...
    public static String loadGraphicsLibrary(String renderer){
        ...
        switch (renderer){
            case "fear_xextream":
                Logger.appendToLog("[FearXextream] Initializing GLES Engine + shadow MapBuffer backend...");
                useGles = true;
                ...
                try {
                    ...
                    Logger.appendToLog("[FearXextream] gl4es_114 backend loaded");
                } catch (Throwable t) {
                    Log.w("JREUtils", "gl4es_114 preload skipped: " + t.getMessage());
                }

                boolean fearRenderLoaded = false;
                try {
                    System.loadLibrary("fear_render");
                    fearRenderLoaded = true;
                    Logger.appendToLog("[FearXextream] fear_render loaded (shadow MapBuffer hooks active)");
                } catch (Throwable t) {
                    Log.e("JREUtils", "fear_render load failed – MapBuffer crash likely", t);
                }
                ...
                try {
                    ...
                    Log.w("JREUtils", "FearXextream engine init optional fail: " + t.getMessage());
                }

                renderLibrary = fearRenderLoaded ? "libfear_render.so" : "libgl4es_114.so";
                Logger.appendToLog("[FearXextream] EGL library = " + renderLibrary);
                break;
            case "vulkan_zink":
