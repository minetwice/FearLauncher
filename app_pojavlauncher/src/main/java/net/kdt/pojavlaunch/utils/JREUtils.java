...
public class JREUtils {
    ...
    public static void setupRendererEnv(Map<String, String> envMap, String renderer) {
        switch(renderer) {
            ...
            case "panvk_zink":
                ...
                envMap.put("MESA_VK_IGNORE_CONFORMANCE_WARNING", "1");
                envMap.put("PANVK_DEBUG", "noafbc");
                try {
                    File icd = new File(Tools.NATIVE_LIB_DIR, "libvulkan_panfrost.so");
                    if (icd.exists()) {
                        envMap.put("VK_DRIVER_FILES", icd.getAbsolutePath());
                        envMap.put("VK_ICD_FILENAMES", icd.getAbsolutePath());
                        Logger.appendToLog("[PanVK] VK_DRIVER_FILES=" + icd.getAbsolutePath());
                    }
    ...
    public static void setEnviroimentForGame(Context context, String renderer) throws Throwable {
        ...
        if ("panvk_zink".equals(renderer)) {
            setUseTurnip(false);
            Logger.appendToLog("[PanVK] setUseTurnip(false) — do not hook Turnip over Panfrost ICD");
    ...
    public static String loadGraphicsLibrary(String renderer){
        ...
        if ("panfork".equals(renderer) || "panvk_zink".equals(renderer)) preloadVk = false;
