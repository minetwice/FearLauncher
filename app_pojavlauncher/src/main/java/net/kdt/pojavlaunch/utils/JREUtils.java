...
public class JREUtils {
    ...
    public static void setupRendererEnv(Map<String, String> envMap, String renderer) {
        switch(renderer) {
            ...
            case "panvk_zink":
            case "vulkan_zink":
                Logger.appendToLog("[" + ("panvk_zink".equals(renderer) ? "PanVKZink" : "TurnipZink")
                        + "] Initializing Zink renderer (OSMesa + Mesa Zink)...");
                ...
                if ("panvk_zink".equals(renderer)) {
                    ...
                    envMap.put("MESA_VK_WSI_DEBUG", "0");
                }
                break;
        }
    }
    public static void setEnviroimentForGame(Context context, String renderer) throws Throwable {
        ...
        boolean isZink = "turnip_zink".equals(renderer) || "panvk_zink".equals(renderer) || "vulkan_zink".equals(renderer);
        if (!isZink) {
        ...
        if ("panvk_zink".equals(renderer)) {
            setUsePanvk(true);
            Logger.appendToLog("[PanVKZink] Using Mesa PanVK Vulkan driver (libvulkan_panfrost.so)");
    ...
    public static String loadGraphicsLibrary(String renderer){
        ...
        switch (renderer){
            ...
            case "vulkan_zink":
                Logger.appendToLog("[" + ("panvk_zink".equals(renderer) ? "PanVKZink" : "TurnipZink")
                        + "] Loading real Mesa OSMesa (libOSMesa_8.so)...");
