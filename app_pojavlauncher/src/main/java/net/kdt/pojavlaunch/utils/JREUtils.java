...
public class JREUtils {
                ...
                try {
                    ...
                    try (BufferedReader reader = new BufferedReader(new InputStreamReader(p.getInputStream(), "UTF-8"), 32768)) {
                        ...
                        while ((line = reader.readLine()) != null) {
                            if (line.contains("jrelog") || line.contains("LIBGL") || line.contains("NativeInput") || line.contains("FEAR") || line.contains("FearRender") || line.contains("FearTurbo") || line.contains("Mesa")) {
    ...
    public static void setupRendererEnv(Map<String, String> envMap, String renderer) {
        switch(renderer) {
            case "fear_turbo":
                Logger.appendToLog("[FearTurbo] High-FPS + shadow MapBuffer environment...");
    ...
    public static void setEnviroimentForGame(Context context, String renderer) throws Throwable {
        ...
        for (Map.Entry<String, String> env : envMap.entrySet()) {
            ...
            try { Os.setenv(env.getKey(), env.getValue(), true); }
            catch (NullPointerException exception) { Log.e("JREUtils", exception.toString()); }
        }
    }
    ...
    public static String loadGraphicsLibrary(String renderer){
        ...
        switch (renderer){
            case "fear_turbo":
                Logger.appendToLog("[FearTurbo] Loading shadow-MapBuffer EGL path...");
                useGles = true;
                bypassNamespace = false;
                glesVersion = 3;

                // 1) gl4es = desktop GL translator backend
                try {
                    System.loadLibrary("gl4es_114");
                    Logger.appendToLog("[FearTurbo] gl4es_114 backend loaded");
                } catch (Throwable t) {
                    Log.w("JREUtils", "gl4es_114 preload: " + t.getMessage());
                }

                // 2) FearTurbo = intercepts glMapBufferRange (CPU shadow + glBufferSubData)
                boolean turboLoaded = false;
                try {
                    System.loadLibrary("FearTurbo");
                    turboLoaded = true;
                    Logger.appendToLog("[FearTurbo] libFearTurbo.so loaded (shadow MapBuffer ACTIVE)");
                    String cachePath = Tools.DIR_GAME_HOME + "/fear_turbo_cache";
                    File cacheDir = new File(cachePath);
                    if (!cacheDir.exists()) cacheDir.mkdirs();
                    initFearTurboEngine(cachePath);
                } catch (Throwable t) {
                    Log.e("JREUtils", "FearTurbo load FAILED — MapBuffer crash likely", t);
                }

                // CRITICAL: EGL must be FearTurbo so LWJGL gets our eglGetProcAddress
                renderLibrary = turboLoaded ? "libFearTurbo.so" : "libgl4es_114.so";
                Logger.appendToLog("[FearTurbo] EGL library = " + renderLibrary);
                break;
            case "vulkan_zink":
    ...
    public static native void initFearTurboEngine(String cachePath);
