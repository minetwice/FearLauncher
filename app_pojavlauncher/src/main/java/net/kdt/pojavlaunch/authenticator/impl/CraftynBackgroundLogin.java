 ...
 * (/authserver/authenticate and /authserver/refresh) - the same protocol
 * Ely.by uses. This does NOT touch the website's /login endpoint (that one
 * issues a website session JWT, not a valid Minecraft access token) and does
 * NOT download/save skin files locally (that never actually reaches the
 * game - authlib-injector handles skin display automatically once
 * AuthType.CRAFTYN_MC.injectorUrl is set correctly).
 *
...
public class CraftynBackgroundLogin implements BackgroundLogin {
    ...
    // Must match AuthType.CRAFTYN_MC's injectorUrl, just with the scheme in
    // front since this is used for direct HTTP calls.
    ...
    private void fillAccount(MinecraftAccount acc, CraftynAuthResponse response) {
        ...
        // and clientToken together, unlike OAuth-style refresh tokens.
